...
- [x] **C3** — Remove `USE_LIVE_DATA` flag, switch to seed-based mock data (types + mock data layer prepared)
...
- [x] **D2** — Wire `compliance_forms` into compliance pack (frontend StateTabs shows real compliance data)
...
*Types, DB layer, and mock data updated. Multi-program ESA and compliance forms wired into StateTabs and /esa page.*

- [x] **F1** — Wire `esa_programs[]` into StateTabs ESA tab (multi-program, platform badges, per-program deadlines)
- [x] **F2** — Wire `compliance_forms` into StateTabs Requirements tab (real notification URLs, assessment rules per state)
- [x] **F3** — Wire `esa_programs[]` into `/esa` page (multiple programs per state card, different platforms)
...
- [x] **F7** — Add `compliance_forms` to Compliance Kit landing page (StateTabs now renders live compliance data)
- [x] **F8** — Wire `esa_programs` + `compliance_forms` through `rowToState()` in `db.ts`
- [x] **F9** — Add `esa_programs` and `compliance_forms` to `StateData` type in `types.ts`
