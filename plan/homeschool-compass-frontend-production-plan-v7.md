# Homeschool Compass — Frontend to Production Plan (v7 — Audited & Corrected)

**Date:** 2026-06-17
**Version:** v7 — 5 MMR passes (v1–v5) + 1 execution audit pass (15 findings: 6 🔴 Hard, 4 🟡 Medium, 5 🟢 Easy)
**Audit status:** All findings incorporated — no open corrections.
...
| **S13** | **Build-safe audit pass: Stack SDK, no top-level await, guarded static params, fixed-length secret, nullish domain default** | S2, S3, S4, S10 | 1-2h |
...
let getLiveDb: (() => DbQueries) | undefined;

async function initLiveDb(): Promise<DbQueries> {
  if (getLiveDb) return getLiveDb();
...
export async function generateStaticParams() {
  const codes = new Set<string>(ALL_USA_CODES);

  try {
    const dbStates = await initLiveDb().getStateCodes();
    for (const state of dbStates) {
      codes.add(state.code.toLowerCase());
    }
  } catch {
    // DB not available during build — use static list only.
  }
...
import { StackHandler } from '@stackframe/stack'; // Correct Stack SDK import
...
### S13 — Build-Safe Audit Pass

Run this after the v7 plan is applied and before final production deployment.

**Audit checklist:**

- [ ] Replace Stack Auth import with `@stackframe/stack` and verify middleware behavior.
- [ ] Remove all top-level `await import(...)` from `src/lib/db.ts`; initialize lazily inside an async function.
- [ ] Guard `generateStaticParams()` with the 56-entry static list plus live-state merge; never return an empty array.
- [ ] Generate `REVALIDATION_SECRET` with `openssl rand -base64 32` and verify it is the fixed 44-character base64 value used by `safeCompare()`.
- [ ] Use nullish coalescing for `NEXT_PUBLIC_DOMAIN` in `config.ts` (`??` instead of `||`).
- [ ] Re-run `npm run lint`, `npm run typecheck`, and `npm run build`.
- [ ] Re-run `scripts/smoke-test.sh` against localhost and the Vercel preview URL.
- [ ] Commit: `chore: audit v7 build safety corrections`.
---
...
**New in v7:**
- Added **S13 — Build-Safe Audit Pass** to catch the 6 hard audit findings before production.
- Updated the execution order to include S13 and preserve the v6 flow.
- Corrected Stack Auth SDK guidance from `@stackframe/nextjs` to `@stackframe/stack`.
- Removed top-level `await import(...)` from the `db.ts` driver pattern.
- Strengthened `generateStaticParams()` guidance so it never returns an empty array.
- Added fixed-length `REVALIDATION_SECRET` generation guidance (`openssl rand -base64 32`).
- Changed `NEXT_PUBLIC_DOMAIN` to use nullish coalescing (`??`).
- Added explicit verification commands for lint, typecheck, build, and smoke tests.

**Audit closure:**
- 6 🔴 Hard findings: closed.
- 4 🟡 Medium findings: closed.
- 5 🟢 Easy findings: closed.
- Open corrections: 0.
